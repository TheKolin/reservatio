		...
		<main>
			<div class="container">
				...
				<form action="add_activities.php" method="POST">
					<div class="row">
						<div class="col">
							<input type="text" class="form-control" placeholder="Nazwa przedmiotu" name="name">
						</div>
						<div class="col">
							<input type="submit" name="submit" value="Dodaj" class="btn btn-light">
						</div>
						<div class="row">
							<p class="h4">Dodane przedmioty</p>
							<?php
								require('input_activities.php');
								$conn -> close();
							?>
						</div>
					</div>
		...
		<script>
			$(document).ready(function(){
				...
				$("table").on('click','#edit_btn',function(){
					...
					value = currentRow.find("td:eq(0)").text();
					//create input
					currentRow.find("td:eq(0)").html('<input class="form-control" type="text" name="new_name" value="">');
					currentRow.find("input").val(value);
					//edit to save
				...
				//back to default
				function cancel(){
					currentRow.find("td:eq(0)").html(''+value);
					currentRow.find("button:eq(1)").remove();
