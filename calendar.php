<?php
	...
	echo '<button name="submit" type="submit" value="0">Poprzedni</button>';
	echo date('Y-m-d', strtotime($mon)).' / '.date('Y-m-d', strtotime($fri));
	echo '<button name="submit" type="submit" value="1">Następny</button>';
	...
	if($last_day < 5){
		for($i=$last_day+$multipler; $i<5; $i++){
			echo "<tr><td>{$dayofweek[$i]}</td></tr>";
		}
	}
    echo '</tr></table>';
